...
字段名 | 类型 | 描述
---|:---:|---
<a name="xmlName"></a>name | `string` | 替换用于描述元素/属性的纲要特性的名称。当在 `items` 内定义时将会影响处于此列表中的每个元素的名称。当定义于 `items` 之上时将会影响它说包裹的元素且仅当 `wrapped` 是 `true` 时，如果 `wrapped` 是 `false` 时它将会被忽略。
